 ...
 * spectgen-analyzer pipeline and run it.
 */
...
/* These should match up with the enum in spectgen.cpp */
  ...
  if (ret == NULL)
    {
      g_print ("Could not create element of type %s, please install it.\n"
	       ...
	       "Also please check that the spectgen package was installed\n"
	       "in the -->same prefix<-- as GStreamer: the spectgen binary\n"
...
main (gint argc, gchar *argv[])
{
  ...
  ctx = g_option_context_new ("[INFILE] - Run spectgen analyzer");
  ...
  if (outfile == NULL) 
    {
      g_print ("Please specify an output .spect file\n\n");
      return RETURN_COMMANDLINE;
    }
  output_file = outfile;
