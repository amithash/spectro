...
static void
gst_spectgen_base_init (gpointer gclass)
{
...
static void
gst_spectgen_class_init (GstSpectgenClass * klass)
{
  ...
  gstelement_class->change_state 
    = GST_DEBUG_FUNCPTR (gst_spectgen_change_state);
}
 ...
 */
static void
gst_spectgen_init (GstSpectgen *spect, GstSpectgenClass *gclass)
{
...
static void
calc_barkband_table (GstSpectgen *spect)
{
  guint i;
  guint barkband = 0;
  ...
  for (i = 0; i < NUMFREQS (spect); ++i)
	...
	barkband++;
      ...
      GST_LOG ("Band %d (frequency %f) -> barkband %d (frequency %d)",
	       i, GST_SPECTRUM_BAND_FREQ (i, spect->size, spect->rate),
...
static gboolean
gst_spectgen_set_sink_caps (GstPad *pad, GstCaps *caps)
{
  GstSpectgen *spect;
...
static gboolean
gst_spectgen_sink_event (GstPad *pad, GstEvent *event)
{
  GstSpectgen *spect;
  gboolean res = TRUE;
...
gst_spectgen_change_state (GstElement *element, GstStateChange transition)
{
...
gst_spectgen_chain (GstPad *pad, GstBuffer *buf)
{
 ...
 */
static void 
gst_spectgen_finish (GstSpectgen *spect)
{
  GstBuffer *buf;
  guchar *data;
  guint line;
  ...
  if (spect->max_width == 0
        || spect->numframes <= spect->max_width)
    output_width = spect->numframes;
  else
    output_width = spect->max_width;

  for(i = 0; i < NBANDS; i++) {
  	normalize(spect->bands[i], spect->numframes);
  }
  buf = gst_buffer_new_and_alloc 
            (output_width * spect->height * NBANDS * sizeof (guchar));
  if (!buf)
    return;
  ...
  gfloat rgb[NBANDS];
  guint start, end;
  for (line = 0; line < spect->height; ++line)
    {
