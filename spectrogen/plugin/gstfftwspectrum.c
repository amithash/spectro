...
#define STREAMDUMP_DEFAULT    FALSE
...
#define OUTPUT_SIZE_DUMP(conv) ((conv)->size *sizeof(float))
...
static void gst_fftwspectrum_2_base_init (gpointer gclass)
{
...
static void gst_fftwspectrum_2_class_init (GstFFTWSpectrumClass * klass)
{
	...
	g_object_class_install_property (gobject_class, ARG_STREAMDUMP,
	g_param_spec_boolean ("streamdump", "Stream Dump", 
		"Dump the stream",
		STREAMDUMP_DEFAULT, G_PARAM_READWRITE));

	gstelement_class->change_state = GST_DEBUG_FUNCPTR (gst_fftwspectrum_2_change_state);
}
 ...
 */
static void gst_fftwspectrum_2_init (GstFFTWSpectrum * conv,
    ...
    const GValue * value, GParamSpec * pspec)
{
	...
	switch (prop_id) {
		...
		case ARG_HIQUALITY:
			...
			break;
		case ARG_STREAMDUMP:
			conv->sdump = g_value_get_boolean (value);
			break;
		default:
    ...
    GValue * value, GParamSpec * pspec)
{
	...
	switch (prop_id) {
		...
		case ARG_HIQUALITY:
			...
			break;
		case ARG_STREAMDUMP:
			g_value_set_boolean (value, conv->sdump);
			break;
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
			break;
...
static GstFlowReturn gst_fftwspectrum_2_chain (GstPad * pad, GstBuffer * buf)
{
	...
	while (conv->numsamples >= MAX (conv->size, conv->step)) {
		if(conv->sdump == FALSE)
			...
			res = gst_pad_alloc_buffer_and_set_caps (
					conv->srcpad, conv->offset, OUTPUT_SIZE_DUMP (conv), 
					GST_PAD_CAPS(conv->srcpad), &outbuf);
		...
		if(conv->sdump == FALSE)
			...
			GST_BUFFER_SIZE       (outbuf) = OUTPUT_SIZE_DUMP (conv);
		...
		if(conv->sdump == FALSE) {
			...
			int i;
			data = (float *)GST_BUFFER_DATA(outbuf);
			for(i = 0; i < conv->size; i++) {
			      data[i] = (float)conv->samples[i];
			}
		}
